...
export class AdminRepository {
      ...
      await query(
        `UPDATE ${table} 
         SET verification_status = $1,
             ...
             updated_at = NOW()
         WHERE id = $3`,
        [status, statusRemarks, entityId]
      );

      // 2. Fetch updated profile to recalculate trust score
      const profileRes = await query(`SELECT * FROM ${table} WHERE id = $1 LIMIT 1`, [entityId]);
      const profile = profileRes.rows[0];
      if (!profile) throw new Error(`${entityType} profile not found.`);

      let trustScore = 0;
      if (entityType === 'builder') {
        trustScore = ProfileScoringService.calculateBuilderTrustScore(profile);
      } else {
        ...
        trustScore = ProfileScoringService.calculateContractorTrustScore(scoreInput);
      }

      // 3. Update trust score
      await query(
        `UPDATE ${table} SET trust_score = $1, updated_at = NOW() WHERE id = $2`,
        [trustScore, entityId]
      );
  ...
  static async suspendUser(userId: string, isSuspended: boolean): Promise<any> {
    await query('BEGIN');
    try {
      await query(
  ...
  static async deleteReview(reviewId: string): Promise<any> {
    ...
    try {
      ...
      const reviewRes = await query('SELECT reviewee_id FROM reviews WHERE id = $1 LIMIT 1', [reviewId]);
      ...
      if (contractor) {
        ...
        await query(
          "UPDATE contractors SET trust_score = $1, updated_at = NOW() WHERE id = $2",
          [trustScore, contractorId]
        );
      }
  ...
  static async cancelPackage(packageId: string): Promise<any> {
    await query('BEGIN');
    try {
      // Update package status to cancelled
      await query(
        "UPDATE project_packages SET status = 'cancelled', updated_at = NOW() WHERE id = $1",
        [packageId]
      );

      // Set associated pending bids to rejected
      await query(
        "UPDATE quotations SET status = 'rejected', updated_at = NOW() WHERE package_id = $1 AND status = 'pending'",
        [packageId]
      );
