...
export class ContractorRepository {
  ...
  static async getProfile(contractorId: string): Promise<any> {
    const contractorRes = await query(
      `SELECT c.*, u.email, u.is_email_verified 
       FROM contractors c
  ...
  static async updateProfile(contractorId: string, profileData: {
    ...
    const docsRes = await query(
      "SELECT file_name FROM documents WHERE entity_type = 'contractor' AND entity_id = $1",
      [contractorId]
    );
    ...
    const trustScore = ProfileScoringService.calculateContractorTrustScore(scoreInput);
    const aiScore = ProfileScoringService.calculateContractorAiScore(
      merged,
  ...
  static async updateSkills(contractorId: string, skillIds: string[]): Promise<any> {
    await query('BEGIN');
    try {
      // 1. Delete current associations
      await query('DELETE FROM contractor_skills WHERE contractor_id = $1', [contractorId]);

      // 2. Insert new associations
      for (const skillId of skillIds) {
        await query(
          'INSERT INTO contractor_skills (contractor_id, skill_id) VALUES ($1, $2)',
          [contractorId, skillId]
        );
      }

      // 3. Recalculate AI score
      const current = await query('SELECT * FROM contractors WHERE id = $1', [contractorId]);
      const contractor = current.rows[0];
      const categoriesRes = await query('SELECT COUNT(*) FROM contractor_categories WHERE contractor_id = $1', [contractorId]);
      const categoriesCount = parseInt(categoriesRes.rows[0].count, 10);

      const aiScore = ProfileScoringService.calculateContractorAiScore(contractor, skillIds.length, categoriesCount);
      await query(
        'UPDATE contractors SET ai_profile_score = $1, updated_at = NOW() WHERE id = $2',
        [aiScore, contractorId]
      );
  ...
  static async updateCategories(contractorId: string, categoryIds: string[]): Promise<any> {
    await query('BEGIN');
    try {
      // 1. Delete current associations
      await query('DELETE FROM contractor_categories WHERE contractor_id = $1', [contractorId]);

      // 2. Insert new associations
      for (const catId of categoryIds) {
        await query(
          'INSERT INTO contractor_categories (contractor_id, category_id) VALUES ($1, $2)',
          [contractorId, catId]
        );
      }
      ...
      const current = await query('SELECT * FROM contractors WHERE id = $1', [contractorId]);
      const contractor = current.rows[0];
      const skillsRes = await query('SELECT COUNT(*) FROM contractor_skills WHERE contractor_id = $1', [contractorId]);
      const skillsCount = parseInt(skillsRes.rows[0].count, 10);

      const aiScore = ProfileScoringService.calculateContractorAiScore(contractor, skillsCount, categoryIds.length);
      await query(
        'UPDATE contractors SET ai_profile_score = $1, updated_at = NOW() WHERE id = $2',
        [aiScore, contractorId]
      );
  ...
  static async submitVerification(
    ...
    await query('BEGIN');
    try {
      const currentRes = await query('SELECT * FROM contractors WHERE id = $1', [contractorId]);
      ...
      await query(
        "DELETE FROM documents WHERE entity_type = 'contractor' AND entity_id = $1",
        [contractorId]
      );
      ...
      for (const doc of verificationData.documents) {
        await query(
          `INSERT INTO documents (entity_type, entity_id, file_name, file_url, file_type) 
           VALUES ('contractor', $1, $2, $3, $4)`,
          [contractorId, doc.name, doc.url, doc.type]
        );
      }
  ...
  static async getDashboardAnalytics(contractorId: string): Promise<any> {
    ...
    const activeCountRes = await query(
      `SELECT COUNT(q.*) 
       ...
       WHERE q.contractor_id = $1 AND q.status = 'accepted' AND pp.status = 'awarded'`,
      [contractorId]
    );
    ...
    const completedCountRes = await query(
      `SELECT COUNT(q.*) 
       FROM quotations q
       JOIN project_packages pp ON q.package_id = pp.id
       WHERE q.contractor_id = $1 AND q.status = 'accepted' AND pp.status = 'completed'`,
      [contractorId]
    );
  ...
  static async getApplicationsHistory(contractorId: string): Promise<any[]> {
    const result = await query(
  ...
  static async getAwardedProjects(contractorId: string): Promise<any[]> {
    const result = await query(
  ...
  static async getReviews(contractorId: string): Promise<any[]> {
    const result = await query(
