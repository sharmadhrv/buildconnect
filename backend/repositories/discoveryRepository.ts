...
export interface BiddingData {
  ...
  breakdown: Array<{ item: string; quantity: number; unit: string; rate: number; total: number }>;
}
...
export class DiscoveryRepository {
  ...
  static async getOpenProjects(filters: {
    ...
    minBudget?: number;
    maxBudget?: number;
    matchingContractorId?: string; // Filter open packages to match contractor's skills
    ...
    const projectResult = await query(projectQuery, values);
    ...
    for (const project of projects) {
      ...
      `;
      const pkgValues: any[] = [project.id];
      ...
      const pkgResult = await query(packageQuery, pkgValues);
  ...
  static async getMatchingPackages(contractorId: string): Promise<any[]> {
  ...
  static async getPackageDetails(packageId: string): Promise<any> {
    const result = await query(
      `SELECT pp.*, 
              p.name as project_name, 
  ...
  static async submitQuotation(
    ...
    const packageRes = await query(
      'SELECT status FROM project_packages WHERE id = $1 LIMIT 1',
      [packageId]
    );
    const pkg = packageRes.rows[0];
    if (!pkg) {
      const err: any = new Error('Project package not found.');
      err.statusCode = 404;
      throw err;
    }
    if (pkg.status !== 'open') {
      const err: any = new Error('Bidding is closed for this project package.');
      err.statusCode = 400;
      throw err;
    }
    ...
    const existingRes = await query(
      'SELECT id FROM quotations WHERE contractor_id = $1 AND package_id = $2 LIMIT 1',
      [contractorId, packageId]
    );
    if (existingRes.rows[0]) {
      const err: any = new Error('You have already submitted a bid for this package.');
      err.statusCode = 409;
      throw err;
    }
    ...
    // 3. Insert quotation
    const result = await query(
      `INSERT INTO quotations (package_id, contractor_id, proposed_budget, proposed_timeline_start, proposed_timeline_end, proposal_notes, breakdown) 
